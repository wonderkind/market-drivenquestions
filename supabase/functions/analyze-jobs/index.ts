...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `You are an expert job market analyst specializing in Dutch/European labor requirements. Analyze job descriptions and extract requirements into four DISTINCT categories.
...
- **Purpose**: Determine if candidate has basic suitability for the role (non-operational)
...
- **Examples**:
  ...
  - Bereidheid om in specifieke omgevingen te werken (bijv. koel/vriescellen of buiten)
 
...
- Fysieke eisen en beschikbaarheid voor shifts zijn OPERATIONELE FIT
...
- **Operationele Fit questions**: Focus on work environment, shift availability, and physical demands
