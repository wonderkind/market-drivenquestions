  ...
  return <Card>
      ...
      <CardContent className="p-4 space-y-4">
        {questions.map((q, index) => <div key={index} className="bg-muted/50 rounded-lg p-4 border border-border/50">
            ...
            {/* Answer Input */}
            {q.answerType && <QuestionAnswer answerType={q.answerType} options={q.options} experienceConfig={q.experienceConfig} value={q.userAnswer} onChange={val => onAnswerChange?.(index, val)} />}
            ...
            {q.quotes && q.quotes.length > 0 && <div className="space-y-2 mb-3">
                ...
                {q.quotes.slice(0, 2).map((quote, i) => <blockquote key={i} className="border-l-2 border-primary/50 pl-3 text-sm italic text-muted-foreground">
                    "{quote}"
                  </blockquote>)}
              </div>}

            {q.sources && q.sources.length > 0 && <div className="flex items-center gap-2 flex-wrap">
                <div className="flex items-center gap-1 text-sm text-muted-foreground border-2">
                  <Building className="h-4 w-4" />
                  Sources:
                </div>
                {q.sources.map((source, i) => <Badge key={i} variant="secondary" className="text-xs text-primary-foreground">
                    {source}
                  </Badge>)}
              </div>}
          </div>)}
