...
import type { Lead } from '@/hooks/useLeadSearch';
  ...
  if (leads.length === 0) {
    return (
      <div className="text-center py-12 text-muted-foreground">
        ...
        <p>No companies found. Select a profile and click "Find Companies" to search.</p>
      </div>
    );
  }
      ...
      <Table>
        ...
        <TableBody>
          {leads.map((lead, index) => (
            <TableRow key={`${lead.employer_name}-${index}`}>
              <TableCell>
              ...
              <TableCell>
                {lead.employer_website ? (
                  ...
                  >
                    {lead.employer_website.replace(/^https?:\/\//, '').replace(/\/$/, '')}
